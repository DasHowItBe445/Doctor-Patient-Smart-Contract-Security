...
contract PatientInfo 
{
    ...
    // Modified function signature to add doctorCaller
    ...
    ) public 
    {
        ...
        {
            // patient themselves can update
        } else if (msg.sender == doctorContractAddress) 
        {
            require(isDoctorAuthorized(patient, doctorCaller), "Doctor not authorized by patient");
        } else 
        {
        ...
        if (bytes(name).length > 0) record.name = name;
        if (age > 0) record.age = age;
        if (bytes(bloodGroup).length > 0) record.bloodGroup = bloodGroup;
        if (bytes(allergies).length > 0) record.allergies = allergies;
        if (bytes(medications).length > 0) record.medications = medications;
        if (bytes(surgeries).length > 0) record.surgeries = surgeries;
        if (bytes(physicianNotes).length > 0) record.physicianNotes = physicianNotes;
    ...
    // New function: returns record fields line-wise as tuple
